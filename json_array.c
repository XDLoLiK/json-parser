...
void json_array_insert(struct JsonArray *json_array, struct JsonValue *value, size_t position) {
    ...
    memmove(&json_array->data[position + 1], &json_array->data[position], json_array->size - position);
    memcpy(&json_array->data[json_array->size++], value, sizeof(struct JsonValue));
}

void json_array_erase(struct JsonArray *json_array, size_t position) {
    position = MIN(json_array->size, position);
    memmove(&json_array->data[position], &json_array->data[position + 1], json_array->size - position);
    json_array->size--;
}
...
void json_array_print(struct JsonArray *json_array, FILE *file) {
    // TODO: create json_value_print() function
}
